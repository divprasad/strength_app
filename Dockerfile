...
#
# The database lives in a Docker volume mounted at /app/prisma/strength_diary.db,
# matching the path that process.cwd() + '/prisma/strength_diary.db' resolves to
...
# (ownership of strength_diary.db itself is fixed by the entrypoint as root before dropping privileges)
RUN mkdir -p /app/prisma && chown nextjs:nodejs /app/prisma

# Run entrypoint as root so it can fix strength_diary.db ownership; it drops to nextjs via su-exec
...
ENV DATABASE_URL="file:./strength_diary.db"
